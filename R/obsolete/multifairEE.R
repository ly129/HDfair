    ...
    verbose = FALSE)
{
  ...
  for (it in seq(maxit)) {
    ...
    ff <- fair_metric(th = th, p = p, M = M, A = A)
    fr <- ff$fair
    if (verbose) cat("fairness = ", fr, "\n")

    # Update slack variable
    U <- pmax(0, pmin(eta_fr, fr + delta/rho_fr)) # pmax(-eta_fr, pmin(eta_fr, fr + delta/rho_fr))
    V <- mapply(function(x, y) {
    ...
    # # increase rho's by a little bit
    # rho_fr <- rho_fr * 1.000001
    # rho_ee <- rho_ee * 1.000001
    ...
    if (sum(fr < eta_fr) == A && max(abs(unlist(ufunc))) < eta_ee * 1.000001 && th_update_norm < eps) break
