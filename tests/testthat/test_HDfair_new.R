...
if (fixed.th) {
  ...
  for (m in seq(M)) {
    ...
    for (a in seq(A)) {
      ...
      epsilon.a <- ((1:p.nz + a) %% 3 - 1) * sd.th.a
...
#
# par(mfrow = c(1, 4))
# plot_sp_lambda(sp_lambda_w)
#
# lasso.p.w <- glmnet(
#   x = X,
#   y = y,
#   family = "gaussian",
#   weights = rep(1/t(table(ma[, 1], ma[, 2])), table(ma[, 1], ma[, 2])),
#   intercept = FALSE,
#   lambda = sp_lambda_w$lambdas * sqrt(M*A)
# )
# plot(lasso.p.w, xvar = "lambda")



sp_lambda_u <- HDfair_sp_lambda(
  X = X,
  y = y,
  ma = ma,
  lambda_length = 20,
...
lasso.p.u <- glmnet(
  ...
  family = "gaussian",
  intercept = FALSE,
  lambda = sp_lambda_u$lambdas * sqrt(M*A)
)
plot(lasso.p.u, xvar = "lambda")
...
# )
# par(mfrow = c(4, 5))
# plot_sp_eta(sp_eta_w, vars = 1:20)

sp_eta_u <- HDfair_sp_eta(
  X = X,
  y = y,
  ma = ma,
  lambda = 0.1,
...
par(mfrow = c(4, 5))
plot_sp_eta(sp_eta_u, vars = 1:20)
